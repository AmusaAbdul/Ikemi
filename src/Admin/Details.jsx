...
const statusMap = { All: null, Pending: "pending_review", Held: "held", Confirmed: "confirmed", Declined: "declined" }

const Details = ({ requests, filter, selectedId, setSelectedId, updateRequest, deleteRequest }) => {
  const filtered = filter === "All" ? requests : requests.filter(r => r.status === statusMap[filter])
  ...
  return (
    <div className="sm:m-5 flex flex-col gap-3 p-4">
      {filtered.length === 0 ?
        <div className="flex min-h-screen space-y-2 items-center  justify-center flex-col">
          <img src={image} className="rounded-full h-35 w-35" alt="No request" />
          <h1 className="flex justify-center items-center text-xl text-custom-gold">No bookings</h1>
        </div> :
        (filtered.map(req => (
        <div key={req.id}>
          <UserRequest request={req} onClick={() => setSelectedId(selectedId === req.id ? null : req.id)} />
          {selectedId === req.id && (
            <div className="espressoBackground p-5 flex justify-between items-center reply-width gap-10 flex-wrap itemSpan WeekSitting">
              <RequestDetails request={req} updateRequest={updateRequest} deleteRequest={deleteRequest} />
            </div>
          )}
        </div>
      )))}
